...
def init_session_state():
    defaults = {
        ...
        # Search Console - Current Month
        "sc_clicks": 450,
        "sc_impressions": 17600,
        "sc_ctr": 2.6,
        "sc_position": 14.3,
        
        # Search Console - Previous Month
        "sc_clicks_prev": 380,
...
class SEOReportPDF(FPDF):
    ...
    def add_table_of_contents(self):
        ...
        self.set_font(self.selected_font, 'B', 22)
        self.set_theme_colors("text_primary")
        ...
        for entry in self.toc_entries:
            indent = (entry["level"] - 1) * 10
            self.set_x(25 + indent)
            
            self.set_theme_colors("text_primary")
            title_width = self.get_string_width(entry["title"])
            self.cell(title_width + 5, 8, entry["title"], 0, 0, 'L')
            
            # Dots
            self.set_theme_colors("text_secondary")
            dots_width = 150 - title_width - indent
            dots = "." * int(dots_width / 2)
            self.cell(dots_width, 8, dots, 0, 0, 'C')
            
            # Page number
            self.set_theme_colors("accent")
            self.cell(15, 8, str(entry["page"]), 0, 1, 'R')
    ...
    def add_technical_seo_section(self, indexed, error_404, crawled, discovered):
        ...
        for i, (label, value, color) in enumerate(error_items):
            ...
            self.set_fill_color(*color[:3], 30)  # Light background
            self.rect(x_error, y, 80, 15, 'F')
...
def generate_pdf(chart_paths, logo_path=None):
    ...
    # Page 2: Search Console Performance
    pdf.add_page()
    pdf.set_y(30)
    ...
    pos_growth = calc_growth(st.session_state.sc_position_prev, st.session_state.sc_position)  # Reversed for position
    ...
    # Generate TOC (insert after cover)
    # Note: TOC is tracked but inserted at end for simplicity
    
    return bytes(pdf.output())


# ============================================================
# 7. CUSTOM CSS FOR STREAMLIT
# ============================================================
st.markdown("""
<style>
    /* Sidebar */
...
# ============================================================
# 8. SIDEBAR - CLIENT & REPORT SETTINGS
# ============================================================
with st.sidebar:
    ...
    st.session_state.report_description = st.text_area(
        ...
        placeholder="Brief overview of this month's SEO activities...",
        key="input_report_desc"
    )
    ...
    st.session_state.client_website = st.text_input(
        "Client Website",
        value=st.session_state.client_website,
        placeholder="www.example.com",
        key="input_client_website"
    )
    ...
    st.session_state.selected_font = st.selectbox(
        ...
        index=["Helvetica", "Times", "Courier"].index(st.session_state.selected_font),
        key="input_font"
    )
    ...
    st.session_state.sc_clicks = st.number_input("Total Clicks", min_value=0, value=st.session_state.sc_clicks, key="sc_clicks_input")
    st.session_state.sc_impressions = st.number_input("Total Impressions", min_value=0, value=st.session_state.sc_impressions, key="sc_impr_input")
    st.session_state.sc_ctr = st.number_input("Average CTR (%)", min_value=0.0, value=st.session_state.sc_ctr, format="%.2f", key="sc_ctr_input")
    st.session_state.sc_position = st.number_input("Average Position", min_value=0.0, value=st.session_state.sc_position, format="%.1f", key="sc_pos_input")
    
    if st.session_state.comparison_enabled:
        st.markdown("**Previous Month:**")
        st.session_state.sc_clicks_prev = st.number_input("Clicks (Prev)", min_value=0, value=st.session_state.sc_clicks_prev, key="sc_clicks_prev")
        st.session_state.sc_impressions_prev = st.number_input("Impressions (Prev)", min_value=0, value=st.session_state.sc_impressions_prev, key="sc_impr_prev")
        st.session_state.sc_ctr_prev = st.number_input("CTR (Prev) %", min_value=0.0, value=st.session_state.sc_ctr_prev, format="%.2f", key="sc_ctr_prev")
        st.session_state.sc_position_prev = st.number_input("Position (Prev)", min_value=0.0, value=st.session_state.sc_position_prev, format="%.1f", key="sc_pos_prev")
    
    st.markdown("---")
    st.header("2️⃣ Revenue & Traffic")
    
    st.session_state.rt_visit = st.number_input("Visits", min_value=0, value=st.session_state.rt_visit, key="rt_visit_input")
    st.session_state.rt_revenue = st.number_input("Total Revenue ($)", min_value=0.0, value=st.session_state.rt_revenue, format="%.2f", key="rt_rev_input")
    st.session_state.rt_impression = st.number_input("Impressions", min_value=0, value=st.session_state.rt_impression, key="rt_impr_input")
    st.session_state.rt_ecommerce = st.number_input("Ecommerce Purchases", min_value=0, value=st.session_state.rt_ecommerce, key="rt_ecom_input")
    st.session_state.rt_purchase_revenue = st.number_input("Purchases Revenue ($)", min_value=0.0, value=st.session_state.rt_purchase_revenue, format="%.2f", key="rt_purch_input")
    st.session_state.rt_transaction = st.number_input("Transactions", min_value=0, value=st.session_state.rt_transaction, key="rt_trans_input")
    
    st.markdown("---")
    st.header("3️⃣ Keyword Ranking")
    
    st.session_state.kw_1_15 = st.number_input("1 to 15 result", min_value=0, value=st.session_state.kw_1_15, key="kw1_input")
    st.session_state.kw_16_30 = st.number_input("16 to 30 result", min_value=0, value=st.session_state.kw_16_30, key="kw2_input")
    st.session_state.kw_31_45 = st.number_input("31 to 45 result", min_value=0, value=st.session_state.kw_31_45, key="kw3_input")
    st.session_state.kw_46_60 = st.number_input("46 to 60 result", min_value=0, value=st.session_state.kw_46_60, key="kw4_input")
    st.session_state.kw_60_100 = st.number_input("60 to 100 result", min_value=0, value=st.session_state.kw_60_100, key="kw5_input")
    
    st.markdown("---")
    st.header("4️⃣ Backlink Profile")
    
    st.session_state.bl_profile = st.number_input("Profile Backlinks", min_value=0, value=st.session_state.bl_profile, key="bl1_input")
    st.session_state.bl_citation = st.number_input("Citation", min_value=0, value=st.session_state.bl_citation, key="bl2_input")
    st.session_state.bl_web2 = st.number_input("Web 2.0", min_value=0, value=st.session_state.bl_web2, key="bl3_input")
    st.session_state.bl_social = st.number_input("Social Share", min_value=0, value=st.session_state.bl_social, key="bl4_input")
    st.session_state.bl_guest = st.number_input("Guest Post", min_value=0, value=st.session_state.bl_guest, key="bl5_input")
    st.session_state.bl_comment = st.number_input("Comment Backlink", min_value=0, value=st.session_state.bl_comment, key="bl6_input")
    
    st.markdown("---")
    st.header("5️⃣ Traffic Analytics")
    
    st.session_state.ta_click = st.number_input("Click", min_value=0, value=st.session_state.ta_click, key="ta1_input")
    st.session_state.ta_impression = st.number_input("Total Impression", min_value=0, value=st.session_state.ta_impression, key="ta2_input")
    st.session_state.ta_ctr = st.number_input("CTR (%)", min_value=0.0, value=st.session_state.ta_ctr, format="%.2f", key="ta3_input")
    st.session_state.ta_position = st.number_input("Average Position", min_value=0.0, value=st.session_state.ta_position, format="%.2f", key="ta4_input")
    
    st.markdown("---")
    st.header("6️⃣ Technical SEO")
    
    st.session_state.tech_indexed = st.number_input("Indexing Page (Valid)", min_value=0, value=st.session_state.tech_indexed, key="tech1_input")
    st.session_state.tech_404 = st.number_input("Not Found (404)", min_value=0, value=st.session_state.tech_404, key="tech2_input")
    st.session_state.tech_crawled_not_indexed = st.number_input("Crawled - Not Indexed", min_value=0, value=st.session_state.tech_crawled_not_indexed, key="tech3_input")
    st.session_state.tech_discovered_not_indexed = st.number_input("Discovered - Not Indexed", min_value=0, value=st.session_state.tech_discovered_not_indexed, key="tech4_input")
...
kw_col1, kw_col2, kw_col3, kw_col4, kw_col5 = st.columns([3, 1, 1, 1, 1])
with kw_col1:
    st.markdown("**Keyword**")
with kw_col2:
...
for i, kw in enumerate(st.session_state.top_keywords):
    ...
    with col1:
        st.session_state.top_keywords[i]["keyword"] = st.text_input(
            f"kw_{i}", value=kw["keyword"], label_visibility="collapsed", key=f"kw_name_{i}"
        )
    with col2:
        st.session_state.top_keywords[i]["position"] = st.number_input(
            f"pos_{i}", value=kw["position"], min_value=1, label_visibility="collapsed", key=f"kw_pos_{i}"
        )
    with col3:
        st.session_state.top_keywords[i]["volume"] = st.number_input(
            f"vol_{i}", value=kw["volume"], min_value=0, label_visibility="collapsed", key=f"kw_vol_{i}"
        )
    with col4:
        st.session_state.top_keywords[i]["difficulty"] = st.number_input(
            f"diff_{i}", value=kw["difficulty"], min_value=0, max_value=100, label_visibility="collapsed", key=f"kw_diff_{i}"
        )
    with col5:
...
with col_preview:
    if st.button("👁️ Preview PDF", use_container_width=True):
        st.info("PDF Preview will be generated...")
